...
#### GitHub Pages 前端发布
仓库包含 `.github/workflows/deploy-pages.yml`。当前仓库 Pages 使用 branch/root 发布方式，推送到 `main` 时 workflow 会执行 `npm ci` 与 `npm run build`，再把 `frontend/build` 同步到仓库根目录的 `index.html`、`404.html`、`.nojekyll` 和 `assets/**`。GitHub 原生 `pages build and deployment` 会发布这些根目录静态文件。

Vite 会在目标仓库 Actions 环境中使用 `/AHR999-Optimized-Kenne-Index/` 作为资源基路径，前端在该环境下使用 `HashRouter`，避免项目页子路径刷新时出现空白页。workflow 已故意不调用 `actions/deploy-pages`，避免 Pages Source 仍为 branch/root 时产生失败检查。
